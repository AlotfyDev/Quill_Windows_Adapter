...
public:
    struct HealthStatus {
        ...
        uint32_t recovery_count;  // AA-M11: Track recovery count
    };

    static void MarkStartup() noexcept;
    static void RecordEmergency() noexcept;
    static void RecordRecovery() noexcept;  // AA-M11: Symmetric recovery tracking
...
private:
    ...
    static std::atomic<uint32_t> recovery_count_;  // AA-M11: Recovery counter
};
...
inline std::atomic<uint32_t> HealthProbe::recovery_count_{0};  // AA-M11: Recovery counter
...
inline void HealthProbe::RecordRecovery() noexcept {
...
inline HealthProbe::HealthStatus HealthProbe::GetHealthStatus() noexcept {
    ...
    status.recovery_count = recovery_count_.load(std::memory_order_acquire);
    return status;
}
